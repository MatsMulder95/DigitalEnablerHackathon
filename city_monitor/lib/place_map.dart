...
class PlaceMapState extends State<PlaceMap> {
  ...
  static BitmapDescriptor _getPlaceMarkerIcon(PlaceCategory category) {
    switch (category) {
      case PlaceCategory.construction:
        return BitmapDescriptor.fromAsset('assets/heart.png');
        break;
      case PlaceCategory.factory:
      default:
  ...
  Future<Map<Marker, Place>> _markPlaces() async {
    await Future.wait(
        AppState
            .of(context)
            .places
            .map((Place place) => _markPlace(place)));
  ...
  Future<void> _markPlace(Place place) async {
    final Marker marker = await mapController.addMarker(
      MarkerOptions(
        ...
        visible: place.category == AppState
            .of(context)
            .selectedCategory,
      ),
    );
  ...
  void _onPlaceChanged(Place value) {
    ...
    final List<Place> newPlaces = List.from(AppState
        .of(context)
        .places);
    final int index =
  ...
  void _confirmAddPlace(BuildContext context) async {
    if (_pendingMarker != null) {
      await mapController.updateMarker(
        _pendingMarker,
        MarkerOptions(
          icon: _getPlaceMarkerIcon(AppState
              .of(context)
              .selectedCategory),
      ...
      return Center(
        child: Stack(
          children: <Widget>[
            ...
            _CategoryButtonBar(
              selectedPlaceCategory: AppState
                  .of(context)
                  .selectedCategory,
    ...
    return Visibility(
      ...
      child: Container(
        ...
        child: ButtonBar(
          ...
          children: <Widget>[
            RaisedButton(
              color: selectedPlaceCategory == PlaceCategory.construction
                  ? Colors.green[700]
                  : Colors.lightGreen,
              child: const Text(
                'Constructions',
              ...
              onPressed: () => onChanged(PlaceCategory.construction),
            ),
            RaisedButton(
              color: selectedPlaceCategory == PlaceCategory.factory
                  ? Colors.green[700]
                  : Colors.lightGreen,
              child: const Text(
                'Factories',
              ...
              onPressed: () => onChanged(PlaceCategory.factory),
            ),
          ],
        ),
      ),
